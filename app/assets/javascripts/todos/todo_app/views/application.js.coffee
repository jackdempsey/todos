((app) ->
  "use strict"
  ApplicationView = Ember.ContainerView.extend(
    ...
    mainView: Em.ContainerView.create(
      ...
      outletView: Ember.View.create(template: Ember.Handlebars.compile("{{outlet}}"))
    ...
    footerView: Ember.ContainerView.create(
      ...
      statsView: Ember.View.create(
        ...
        oneLeft: (->
          @getPath("entries.remaining") is 1
      ...
      filtersView: Ember.View.create(
        ...
        isAll: (->
          Ember.empty @get("filter")
        ).property("filter")
        isActive: (->
          @get("filter") is "active"
        ).property("filter")
        isCompleted: (->
          @get("filter") is "completed"
        ).property("filter")
      )
